...
namespace Services.CAR
{
    internal sealed class DynamicFormConfigurationService(IDataManager data, ICacheManager memCache, ILocalizationService localization) : IDynamicFormConfigurationService
    {
        ...
        public async Task<ApiResponse<IEnumerable<DynamicFormConfigurationDto>>> GetDynamicFormConfiguration(string Language, string Plant, string? search, bool delflag, string? formTypeAdv, string? fieldNameAdv, string? fieldTypeAdv, string? fieldLengthAdv, string? mandatoryAdv, string? fieldElementAdv, string? optionDataResourceAdv, string? dbResourceAdv, string? queryAdv, string? dataOptionAdv, string? sequenceAdv)
        {
            var result = await data.DynamicFormConfiguration.GetDynamicFormConfiguration(Language, Plant, search, delflag, formTypeAdv, fieldNameAdv, fieldTypeAdv, fieldLengthAdv, mandatoryAdv, fieldElementAdv, optionDataResourceAdv, dbResourceAdv, queryAdv, dataOptionAdv, sequenceAdv);
            return ApiResponse<IEnumerable<DynamicFormConfigurationDto>>.SuccessResponse(result);
        }
