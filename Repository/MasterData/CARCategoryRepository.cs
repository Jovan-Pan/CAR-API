...
namespace Repository.MasterData
{
        internal sealed class CARCategoryRepository(DbContext dbContext) : ICARCategoryRepository
        {
            ...
            {
                string query = CARCategoryQuery.InsertNewCARCategory;
                await using var conn = dbContext.CARConnection();
                return await conn.QueryAsync<CARCategoryDto>(query, new { CARCategory = CRUDCARCategoryDto.CARCategory, userId = CRUDCARCategoryDto.userid, plant = CRUDCARCategoryDto.Plant });
            }

            public async Task<IEnumerable<CARCategoryDto>> UpdateCARCategory(CRUDCARCategoryDto CRUDCARCategoryDto)
            {
                string query = CARCategoryQuery.UpdateCARCategory;
                await using var conn = dbContext.CARConnection();
                return await conn.QueryAsync<CARCategoryDto>(query, new { CARCategory = CRUDCARCategoryDto.CARCategory, id = CRUDCARCategoryDto.id, userId = CRUDCARCategoryDto.userid });
            }

            public async Task<IEnumerable<CARCategoryDto>> DataDelete(CRUDCARCategoryDto CRUDCARCategoryDto)
            {
                string query = CARCategoryQuery.DeleteCARCategory;
                await using var conn = dbContext.CARConnection();
                return await conn.QueryAsync<CARCategoryDto>(query, new { id = CRUDCARCategoryDto.id, userId = CRUDCARCategoryDto.userid });
            }

            public async Task<IEnumerable<CARCategoryDto>> DataPermDelete(CRUDCARCategoryDto CRUDCARCategoryDto)
            {
                string query = CARCategoryQuery.PermDeleteCARCategory;
                await using var conn = dbContext.CARConnection();
                return await conn.QueryAsync<CARCategoryDto>(query, new { id = CRUDCARCategoryDto.id });
            }

            public async Task<IEnumerable<CARCategoryDto>> DataRecover(CRUDCARCategoryDto CRUDCARCategoryDto)
            {
                string query = CARCategoryQuery.RecoverCARCategory;
                await using var conn = dbContext.CARConnection();
                return await conn.QueryAsync<CARCategoryDto>(query, new { id = CRUDCARCategoryDto.id, userId = CRUDCARCategoryDto.userid });
            }
            ...
            public async Task<byte[]> Template()
            {
                ...
                string fileName = string.Format("CARCategory_{0}.xlsx", DateTime.Now.ToString("yyyyMMddHHmmss"));
            ...
            private static void WriteTemplateContent(ExcelWorksheet sheet)
            {
                ...
                var row4Header = new object[] { "Plant","CARCategory" };
        ...
        public async Task<ImportResult> Import(string filePath, string userId)
        {
            ...
            ExcelReadResponseDto excelData = GlobalFunction.ReadExcelFile(filePath, userId, query, excelCol, "CARCategory", "CARCategory", conditions, condRemark, excelRange, uniqueField);
