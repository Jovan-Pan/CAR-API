...
namespace Repository.MasterData
{
    internal sealed class RiskCategoryRepository(DbContext dbContext) :IRiskCategoryRepository
    {
        ...
        {
            string query = RiskCategoryQuery.InsertNewData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<RiskCategoryDto>(query, new { RiskCategory = CRUDRiskCategoryDto.RiskCategory, userId = CRUDRiskCategoryDto.userid, plant = CRUDRiskCategoryDto.Plant });
        }

        public async Task<IEnumerable<RiskCategoryDto>> UpdateData(CRUDRiskCategoryDto CRUDRiskCategoryDto)
        {
            string query = RiskCategoryQuery.UpdateData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<RiskCategoryDto>(query, new { RiskCategory = CRUDRiskCategoryDto.RiskCategory, id = CRUDRiskCategoryDto.id, userId = CRUDRiskCategoryDto.userid });
        }

        public async Task<IEnumerable<RiskCategoryDto>> DataDelete(CRUDRiskCategoryDto CRUDRiskCategoryDto)
        {
            string query = RiskCategoryQuery.DeleteData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<RiskCategoryDto>(query, new { id = CRUDRiskCategoryDto.id, userId = CRUDRiskCategoryDto.userid });
        }

        public async Task<IEnumerable<RiskCategoryDto>> DataPermDelete(CRUDRiskCategoryDto CRUDRiskCategoryDto)
        {
            string query = RiskCategoryQuery.PermDeleteData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<RiskCategoryDto>(query, new { id = CRUDRiskCategoryDto.id });
        }

        public async Task<IEnumerable<RiskCategoryDto>> DataRecover(CRUDRiskCategoryDto CRUDRiskCategoryDto)
        {
            string query = RiskCategoryQuery.RecoverData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<RiskCategoryDto>(query, new { id = CRUDRiskCategoryDto.id, userId = CRUDRiskCategoryDto.userid });
        }
        ...
        public async Task<byte[]> Template()
        {
            ...
            string fileName = string.Format("RiskCategory_{0}.xlsx", DateTime.Now.ToString("yyyyMMddHHmmss"));
        ...
        private static void WriteTemplateContent(ExcelWorksheet sheet)
        {
            ...
            var row4Header = new object[] { "Plant", "RiskCategory" };
        ...
        {
            string excelCol = "Plant,RiskCategory ";
            string excelRange = "A4:E5000";
            string query = RiskCategoryQuery.Import;
            ...
            ExcelReadResponseDto excelData = GlobalFunction.ReadExcelFile(filePath, userId, query, excelCol, "RiskCategory", "RiskCategory", conditions, condRemark, excelRange, uniqueField);
