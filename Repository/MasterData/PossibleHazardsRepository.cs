...
namespace Repository.MasterData
{
    internal sealed class PossibleHazardsRepository(DbContext dbContext) : IPossibleHazardsRepository
    {
        ...
        {
            string query = PossibleHazardsQuery.InsertNewData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<PossibleHazardsDto>(query, new { PossibleHazards = CRUDPossibleHazardsDto.PossibleHazards, userId = CRUDPossibleHazardsDto.userid, plant = CRUDPossibleHazardsDto.Plant });
        }

        public async Task<IEnumerable<PossibleHazardsDto>> UpdateData(CRUDPossibleHazardsDto CRUDPossibleHazardsDto)
        {
            string query = PossibleHazardsQuery.UpdateData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<PossibleHazardsDto>(query, new { PossibleHazards = CRUDPossibleHazardsDto.PossibleHazards, id = CRUDPossibleHazardsDto.id, userId = CRUDPossibleHazardsDto.userid });
        }

        public async Task<IEnumerable<PossibleHazardsDto>> DataDelete(CRUDPossibleHazardsDto CRUDPossibleHazardsDto)
        {
            string query = PossibleHazardsQuery.DeleteData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<PossibleHazardsDto>(query, new { id = CRUDPossibleHazardsDto.id, userId = CRUDPossibleHazardsDto.userid });
        }

        public async Task<IEnumerable<PossibleHazardsDto>> DataPermDelete(CRUDPossibleHazardsDto CRUDPossibleHazardsDto)
        {
            string query = PossibleHazardsQuery.PermDeleteData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<PossibleHazardsDto>(query, new { id = CRUDPossibleHazardsDto.id });
        }

        public async Task<IEnumerable<PossibleHazardsDto>> DataRecover(CRUDPossibleHazardsDto CRUDPossibleHazardsDto)
        {
            string query = PossibleHazardsQuery.RecoverData;
            await using var conn = dbContext.CARConnection();
            return await conn.QueryAsync<PossibleHazardsDto>(query, new { id = CRUDPossibleHazardsDto.id, userId = CRUDPossibleHazardsDto.userid });
        }
        ...
        public async Task<byte[]> Template()
        {
            ...
            string fileName = string.Format("PossibleHazards_{0}.xlsx", DateTime.Now.ToString("yyyyMMddHHmmss"));
        ...
        private static void WriteTemplateContent(ExcelWorksheet sheet)
        {
            ...
            var row4Header = new object[] { "Plant", "PossibleHazards" };
        ...
        {
            string excelCol = "Plant,PossibleHazards ";
            string excelRange = "A4:E5000";
            string query = PossibleHazardsQuery.Import;
            ...
            ExcelReadResponseDto excelData = GlobalFunction.ReadExcelFile(filePath, userId, query, excelCol, "PossibleHazards", "PossibleHazards", conditions, condRemark, excelRange, uniqueField);
